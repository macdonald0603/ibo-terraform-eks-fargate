...
resource "aws_db_subnet_group" "ibo_prod_app_subnet_group" {
  name       = "ibo-prod-app-db-subnet-group"
...
resource "aws_security_group" "ibo_prod_app_db_sg" {
  name   = "ibo-prod-app-db-sg"
  vpc_id = aws_vpc.ibo_prod_app.id  # Corrected VPC reference
...
resource "aws_db_instance" "ibo_prod_app_db" {
  allocated_storage       = 20
  storage_type            = "gp2"
  engine                  = "mysql"   # Change to your desired database engine
  engine_version          = "8.0"     # Specify the desired version
  instance_class          = "db.t3.micro"
  db_name                 = "iboproddb"   # Corrected to use db_name
  username                = "admin"      # Username for the database
  password                = "password123" # Replace with a secure password
  db_subnet_group_name    = aws_db_subnet_group.ibo_prod_app_subnet_group.name
  vpc_security_group_ids  = [aws_security_group.ibo_prod_app_db_sg.id]  # Reference to the security group for RDS

  skip_final_snapshot = true

  tags = {
    Name = "ibo_prod_app_db"
  }
}
